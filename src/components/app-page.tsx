...
const suggestionPrompts = [
  "What do you think about FTX US being solvent? Can you elaborate on the potential implications for customers and the crypto market as a whole?",
  "Can you explain the significance of the $428m bank balances and how they relate to FTX's overall financial situation?",
  "What are your thoughts on the control of the system being turned off? How might this affect the investigation and recovery process?",
  "How do you feel about the S&C files claiming FTX US is insolvent? What evidence supports or contradicts this claim?",
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-purple-600 via-pink-500 to-red-500 text-white">
      <header className="bg-white bg-opacity-10 backdrop-blur-lg shadow-lg">
        <div className="max-w-6xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-pink-600">
            Chat with SBF AI
          </h1>
        </div>
      </header>
      <main className="flex-grow max-w-4xl w-full mx-auto p-4 sm:p-6">
        <Card className="mb-4 sm:mb-6 bg-white bg-opacity-10 backdrop-blur-md border-none rounded-2xl sm:rounded-3xl overflow-hidden">
          <CardContent className="p-4 sm:p-6">
            <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-yellow-300">Suggested Topics</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
              {suggestionPrompts.map((prompt, index) => (
                <Button
                  ...
                  disabled={isLoading}
                  className="bg-white bg-opacity-20 text-white border-none hover:bg-white hover:bg-opacity-30 transition-colors duration-200 rounded-xl sm:rounded-2xl h-auto py-2 sm:py-3 px-3 sm:px-4 text-xs sm:text-sm font-medium text-left overflow-hidden"
                >
        ...
        <Card className="mb-4 sm:mb-6 bg-white bg-opacity-10 backdrop-blur-md border-none rounded-2xl sm:rounded-3xl overflow-hidden">
          <CardContent className="p-0">
            <ScrollArea className="h-[300px] sm:h-[400px] md:h-[500px] lg:h-[600px] p-4 sm:p-6 scroll-area">
              {messages.map((message, index) => (
                <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} mb-4 sm:mb-6`}>
                  <div className={`flex items-start max-w-[85%] sm:max-w-[75%] ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    <Avatar className="w-8 h-8 sm:w-10 sm:h-10 mr-2 sm:mr-3 flex-shrink-0">
                      <AvatarImage src={message.role === 'user' ? '/user-avatar.png' : '/sbf-avatar.png'} />
                      <AvatarFallback>{message.role === 'user' ? 'U' : 'SBF'}</AvatarFallback>
                    </Avatar>
                    <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl text-sm sm:text-base ${
                      message.role === 'user' 
                        ? 'bg-yellow-500 bg-opacity-80 text-gray-900' 
                        : 'bg-white bg-opacity-20 text-white'
                    }`}>
                      {message.isLoading ? (
                        <div className="flex items-center space-x-2">
                          <Loader2 className="h-4 w-4 sm:h-5 sm:w-5 animate-spin text-yellow-300" />
                          <span className="text-yellow-300 text-sm sm:text-base">Thinking...</span>
                        </div>
                      ) : (
                        message.content
                      )}
        ...
        <div className="flex space-x-2 sm:space-x-3">
          <Input
            ...
            className="flex-grow bg-white bg-opacity-20 text-white border-none placeholder-gray-300 rounded-full py-2 sm:py-3 px-4 sm:px-5 text-sm sm:text-base"
            disabled={isLoading}
          />
          <Button 
            onClick={() => handleSendMessage(inputMessage)} 
            disabled={isLoading}
            className="bg-yellow-500 text-gray-900 hover:bg-yellow-400 rounded-full p-2 sm:p-3"
          >
            {isLoading ? <Loader2 className="h-4 w-4 sm:h-5 sm:w-5 animate-spin" /> : <Send className="h-4 w-4 sm:h-5 sm:w-5" />}
          </Button>
        </div>
      </main>
      <footer className="py-4 text-center text-white text-xs sm:text-sm">
